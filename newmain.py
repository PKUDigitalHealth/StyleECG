...
from Convolution.Pixel_Offset_Extractor.run import generate_pixel_offset_extractor
from Convolution.Style_Enhancer.run import generate_style_enhancer

def newwrinkle(style_path,cont_path,output_path, pixel_offset_extractor_save_path, device):
    ...
    H, W = 25, 25                       
    s_path = (bgray.reshape(1100//H, H, 1100//W, W)
          .swapaxes(1, 2))               
    s_path = s_path.reshape(-1, H, W)[:, None, :, :]  
    s_path = torch.tensor(s_path, dtype=torch.float32, device=device)
    goffset2 = generate_pixel_offset_extractor(s_path, pixel_offset_extractor_save_path, device=device)
    goffset2 = goffset2.detach().cpu().numpy()
    goffset2 = (goffset2.squeeze(1)        
               .reshape(44, 44, H, W)
               .swapaxes(1, 2)
               .reshape(1100, 1100))
               
    goffset2 = np.round(goffset2, 1)   
...
if __name__ == '__main__':
    ...
    pixel_offset_extractor_save_path = "/data/wangkexin/changecode/Convolution/Pixel_Offset_Extractor/checkpoint/best_model.pth"
    style_enhencer_save_path = "/data/wangkexin/changecode/Convolution/Style_Enhance/best_model.pth"
    ...
    for index, file in enumerate(ecgall_file):#The pictures are processed one by one. Read one by one, and then generate one by one
        ...
        dis_path = newwrinkle(sty_path,ecg_path,sampled_dir + "dis_" + file, pixel_offset_extractor_save_path, device = "cuda:" + str(args.device))
        ...
        styecg = eval(modelConfig, model,ecgall,styall,disall)
        
        print(type(styecg),type(disall))
        print(len(styecg), len(disall))
        print(tyle(styecg[0]), tyle(disall[0]))
        
        wriecgall = generate_style_enhancer(styecg, styall, style_enhencer_save_path, "cuda:" + str(args.device))
        wriecg = combine_tiles(wriecgall, sampled_dir + file, (32,32))
        wriecg.save(sampled_dir + "wri_" +file)
